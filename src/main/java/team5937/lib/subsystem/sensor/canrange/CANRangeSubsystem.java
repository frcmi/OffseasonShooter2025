package team5937.lib.subsystem.sensor.canrange;
...
public class CANRangeSubsystem extends RegisteredSubsystem {
    ...
    public void periodic() {
        ...
        LoggedTunableNumber.ifChanged(
                hashCode(),
                () -> config.setThreshold(Inches.of(thresholdTunable.get())),
                thresholdTunable);
        LoggedTunableNumber.ifChanged(
                hashCode(),
                () -> {
                    ...
                    withinThreshold =
                            new Trigger(() -> isWithinThreshold)
                                    .debounce(config.getDebounce().in(Seconds));
                },
                debounceTunable);

        Logger.recordOutput(
                String.format("CANRanges/%s/WithinThreshold", logKey), isWithinThreshold());
        Logger.recordOutput(
                String.format("CANRanges/%s/ThresholdInches", logKey),
                config.getThreshold().in(Inches));
        Logger.recordOutput(
